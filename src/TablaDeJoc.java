public class TablaDeJoc {
    ...
    public void descoperaCelula(int l, int c) {
        // Conditie de oprire: in afara tablei
        if (l < 0 || l >= linii || c < 0 || c >= coloane) {
            return;
        }

        // Conditie de oprire: deja descoperita sau are steag
        if (grid[l][c].esteDescoperit() || grid[l][c].areSteag()) {
            return;
        }

        // Descoperim celula in logica din spate
        grid[l][c].setEsteDescoperit(true);

        // Conditie de oprire a propagarii: daca dam de un numar (> 0 vecini)
        if (grid[l][c].getBombeVecine() > 0) {
            return;
        }

        // Propagarea recursiva pe cei 8 vecini
        for (int dinamicL = -1; dinamicL <= 1; dinamicL++) {
            for (int dinamicC = -1; dinamicC <= 1; dinamicC++) {
    ...
    public boolean verificaVictorie() {
        for (int i = 0; i < linii; i++) {
            for (int j = 0; j < coloane; j++) {
                // Daca gasim o celula care NU este minata, dar a ramas NEDESCOPERITa,
                // inseamna ca jucatorul nu a terminat inca de curatat tabla.
                if (!grid[i][j].esteMinat() && !grid[i][j].esteDescoperit()) {
                    return false;
                }
            }
        }
        // Daca am parcurs toata tabla si nu am gasit nicio celula sigura ascunsa...
        return true;
    }
}